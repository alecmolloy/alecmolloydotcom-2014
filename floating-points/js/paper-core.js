                ...
                return Base.deserialize(
                    ...
                    function (type, args, isRoot) {
                        var obj = target && target.constructor === type ? target : Base.create(type.prototype),
                            isTarget = obj === target;
                        if (!isRoot && args.length === 1 && obj instanceof Item && (!(obj instanceof Layer) || isTarget)) {
                            var arg = args[0];
    ...
    var Callback = {
        ...
        fire: function (type, event) {
            ...
            for (var i = 0, l = handlers.length; i < l; i++) {
                if (handlers[i].apply(that, args) === false && event && event.stop) {
                    event.stop();
                    break;
                }
            }
  ...
  [0.1894506104550684962853967, 0.1826034150449235888667637, 0.1691565193950025381893121, 0.1495959888165767320815017, 0.1246289712555338720524763, 0.0951585116824927848099251, 0.0622535239386478928628438, 0.0271524594117540948517806]
   ...
   'LeftCenter', 'TopCenter', 'RightCenter', 'BottomCenter'],
    ...
    var Matrix = Base.extend({
     ...
     f.number(this._tx)].join(', ') + '], [' + [f.number(this._c), f.number(this._d),
    ...
    var Project = PaperScopeItem.extend({
        ...
        addChild: function (child) {
            if (child instanceof Layer) {
                ...
                (this.activeLayer || this.addChild(new Layer(Item.NO_INSERT))).addChild(child);
            } else {
                child = null;
            }
            return child;
    ...
    var Item = Base.extend(Callback, {
        ...
        _initialize: function (props, point) {
            ...
            return props && props !== Item.NO_INSERT ? this._set(props, {
                insert: true
            }) : true;
        },
  ...
  'internalBounds', 'internalRoughBounds'],
        function (key) {
            ...
            this[getter] = function (_matrix) {
                var boundsGetter = this._boundsGetter,
                    ...
                    bounds = this._getCachedBounds(name, _matrix, null,
                        internalGetter);
                return key === 'bounds' ? new LinkedRectangle(bounds.x, bounds.y, bounds.width,
                    bounds.height, this, 'setBounds') : bounds;
            };
        }, {
            beans: true,
            ...
            _getCachedBounds: function (getter, matrix, cacheItem, internalGetter) {
                ...
                matrix = !matrix ? _matrix : _matrix ? matrix.clone().concatenate(_matrix) : matrix;
            ...
            statics: {
                _clearBoundsCache: function (item) {
                    if (item._boundsCache) {
                        for (var i = 0, list = item._boundsCache.list, l = list.length; i < l; i++) {
                            var child = list[i];
        ...
        getGlobalMatrix: function (_internal) {
            ...
            if (!matrix) {
                ...
                matrix.preConcatenate(this._parent ? this._parent.getGlobalMatrix(true) : viewMatrix);
    ...
    '_clipMask', '_guide', '_applyMatrix'];
        ...
        hitTest: function (point, options) {
            ...
            var matrix = this._matrix,
                ...
                view = this.getView(),
                totalMatrix = options._totalMatrix = parentTotalMatrix ? parentTotalMatrix.clone().concatenate(matrix) : this.getGlobalMatrix().clone().preConcatenate(
                    view._matrix),
            ...
            if (!this._children && !this.getInternalRoughBounds()
                ...
                return null;
            var type,
                checkSelf = !(options.guides && !this._guide || options.selected && !this._selected || (type = options.type) && (typeof type === 'string' ? type !== Base.hyphenate(this._class) : !(this instanceof type))),
                that = this,
                res;
     ...
     'LeftCenter', 'TopCenter', 'RightCenter', 'BottomCenter'
        ...
        draw: function (ctx, param) {
            ...
            var blendMode = this._blendMode,
                ...
                direct = normalBlend && opacity === 1 || param.clip || (nativeBlend || normalBlend && opacity < 1) && this._canComposite(),
        ...
        _remove: function _remove(notify) {
            ...
            if (this._index != null) {
                if (this._project.activeLayer === this)
                    this._project.activeLayer = this.getNextSibling() || this.getPreviousSibling();
            ...
            return {
                _contains: function _contains(point) {
                    if (this._type === 'rectangle') {
                        ...
                        return center ? point.subtract(center).divide(this._radius)
                            .getLength() <= 1 : _contains.base.call(this, point);
                    } else {
    ...
    var Raster = Item.extend({
        ...
        getAverageColor: function (object) {
            ...
            ctx.drawImage(this.getElement(), -this._size.width / 2, -this._size.height / 2);
            ctx.restore();
    ...
    var Segment = Base.extend({
        ...
        _serialize: function (options) {
            return Base.serialize(this.isLinear() ? this._point : [this._point, this._handleIn, this._handleOut],
                options, true);
        },
        ...
        _changed: function (point) {
            ...
            var curves = path._curves,
                ...
                curveIn, curveOut;
            if (curves) {
                if ((!point || point === this._point || point === this._handleIn) && (curveIn = curves[index - 1] || path._closed && curves[curves.length - 1]))
                    curveIn._changed();
                if ((!point || point === this._point || point === this._handleOut) && (curveOut = curves[index]))
                    curveOut._changed();
            }
            path._changed(25);
        },
        ...
        isColinear: function (segment) {
            ...
            return this._handleOut.isZero() && next1._handleIn.isZero() && segment._handleOut.isZero() && next2._handleIn.isZero() && next1._point.subtract(this._point).isColinear(
        ...
        isOrthogonal: function () {
            var prev = this.getPrevious(),
                next = this.getNext();
            return prev._handleOut.isZero() && this._handleIn.isZero() && this._handleOut.isZero() && next._handleIn.isZero() && this._point.subtract(prev._point).isOrthogonal(
        ...
        isArc: function () {
            ...
            if (handle1.isOrthogonal(handle2)) {
                ...
                return corner && Numerical.isZero(handle1.getLength() /
                    corner.subtract(from).getLength() - kappa) && Numerical.isZero(handle2.getLength() /
    ...
    var Curve = Base.extend({
            ...
            split: function (offset, isParameter) {
                return this._path ? this._path.split(this._segment1._index,
                    this._getParameter(offset, isParameter)) : null;
            },
   ...
   [p8x, p8y, p7x, p7y, p5x, p5y, p2x, p2y]
                ...
                getArea: function (v) {
                    var p1x = v[0],
                        ...
                        p2x = v[6],
                        p2y = v[7];
                    return (3.0 * c1y * p1x - 1.5 * c1y * c2x - 1.5 * c1y * p2x - 3.0 * p1y * c1x - 1.5 * p1y * c2x - 0.5 * p1y * p2x + 1.5 * c2y * p1x + 1.5 * c2y * c1x - 3.0 * c2y * p2x + 0.5 * p2y * p1x + 1.5 * p2y * c1x + 3.0 * p2y * c2x) / 10;
                },
                ...
                _addBounds: function (v0, v1, v2, v3, coord, padding, min, max, roots) {
                    ...
                    for (var i = 0; i < count; i++) {
                        var t = roots[i],
                            u = 1 - t;
                        if (tMin < t && t < tMax)
                            add(u * u * u * v0 + 3 * u * u * t * v1 + 3 * u * t * t * v2 + t * t * t * v3,
                                padding);
                    }
                }
            }
            ...
            return {
                statics: {
                    getIntersections: function (v1, v2, curve1, curve2, locations, include) {
                        ...
                        (linear1 && linear2 ? addLineIntersection : linear1 || linear2 ? addCurveLineIntersections : addCurveIntersections)(
    ...
    var CurveLocation = Base.extend({
        ...
        equals: function (loc) {
            var isZero = Numerical.isZero;
            return this === loc || loc && this._curve === loc._curve && this._curve2 === loc._curve2 && isZero(this._parameter - loc._parameter) && isZero(this._parameter2 - loc._parameter2) || false;
        },
        ...
        getIntersections: function (path, _expand) {
            ...
            for (var i = 0; i < length1; i++) {
                ...
                for (var j = path ? 0 : i + 1; j < length2; j++) {
                    Curve.getIntersections(
                        values1, values2[j], curve1, curves2[j], locations, !path && (j === i + 1 || j === length2 - 1 && i === 0) && function (loc) {
                ...
                case 'a':
                    for (var j = 0; j < length; j += 7) {
                        this.arcTo(current = getPoint(j + 5),
                            new Size(+coords[0], +coords[1]), +coords[2], +coords[4], +coords[3]);
                    }
                    break;
                case 'z':
                    this.closePath(true);
    ...
    var Path = PathItem.extend({
        ...
        createSegments: function () {
            ...
            for (i = 0; i < segments; i++) {
                // Divide a path into 10 segments
                var x1 = this.segments[0].point.x;
                var y1 = this.segments[0].point.y;
                var x2 = this.segments[1].point.x;
                var y2 = this.segments[1].point.y;

                var segmentX = ((((x2 - x1) / segments) * i) + x1);
                var segmentY = ((((y2 - y1) / segments) * i) + y1);

                this.segmentArray[i] = new paper.Point(segmentX, segmentY);
            }
            return workPath;
        },
        ...
        toShape: function (insert) {
            ...
            if (this.isPolygon() && segments.length === 4 && isColinear(0, 2) && isColinear(1, 3) && isOrthogonal(1)) {
                type = Shape.Rectangle;
                size = new Size(getDistance(0, 3), getDistance(0, 1));
                topCenter = segments[1]._point.add(segments[2]._point).divide(2);
            } else if (segments.length === 8 && isArc(0) && isArc(2) && isArc(4) && isArc(6) && isColinear(1, 5) && isColinear(3, 7)) {
                type = Shape.Rectangle;
        ...
        _hitTest: function (point, options) {
            ...
            function checkSegmentPoints(seg, ends) {
                return (ends || options.segments) && checkSegmentPoint(seg, seg._point, 'segment') || (!ends && options.handles) && (
            ...
            smooth: function () {
                ...
                for (var i = overlap; i <= n - overlap; i++) {
                    ...
                    if (i < n) {
                        ...
                        handleIn = i < n - 1 ? new Point(
                            2 * knots[i + 1]._x - x[i + 1],
                            2 * knots[i + 1]._y - y[i + 1]) : new Point(
        ...
        statics: {
            ...
            getStrokeBounds: function (segments, closed, style, matrix) {
                ...
                function addJoin(segment, join) {
                    ...
                    if (join === 'round' || !handleIn.isZero() && !handleOut.isZero() && handleIn.isColinear(handleOut)) {
                        addRound(segment);
                    } else {
   ...
   new Segment([0, 1], [kappa, 0], [-kappa, 0])
    ...
    var CompoundPath = PathItem.extend({
        ...
        _getChildHitTestOptions: function (options) {
            return options.type === 'path' ? options : new Base(options, {
                fill: false
            });
        },
            ...
            for (var i = 0, l = segments.length; i < l; i++) {
                ...
                for (var j = 0; j < 3; j++) {
                    ...
                    if (path._parent instanceof CompoundPath)
                        path = path._parent;
                    windings[j] = subtract && _path2 && (path === _path1 && _path2._getWinding(point, hor) || path === _path2 && !_path1._getWinding(point, hor)) ? 0 : getWinding(point, monoCurves, hor);
                }
                windings.sort();
        ...
        function splitPath(intersections) {
            ...
            for (var i = intersections.length - 1, curve, prevLoc; i >= 0; i--) {
                var loc = intersections[i],
                    t = loc._parameter;
                if (prevLoc && prevLoc._curve === loc._curve && prevLoc._parameter > 0) {
                ...
                if (newCurve = curve.divide(t, true, true)) {
                    ...
                    segment = t < TOLERANCE ? curve._segment1 : t > 1 - TOLERANCE ? curve._segment2 : curve.getPartLength(0, t) < curve.getPartLength(t, 1) ? curve._segment1 : curve._segment2;
                }
            ...
            if (horizontal) {
                ...
                for (var i = 0, l = curves.length; i < l; i++) {
                    var curve = curves[i],
                        ...
                        next = curve.next;
                    if (winding && (winding === 1 && y >= values[1] && y <= values[7] || y >= values[7] && y <= values[1]) && Curve.solveCubic(values, 1, y, roots, 0, !next.winding && next.values[1] === y ? 1 : MAX) === 1) {
                        var t = roots[0],
                            x0 = Curve.evaluate(values, t, 0).x,
                            slope = Curve.evaluate(values, t, 1).y;
                        if (abs(slope) < TOLERANCE && !Curve.isLinear(values) || t < TOLERANCE && slope * Curve.evaluate(
                        ...
                        if (selfOp) {
                            ...
                            if (w3 * w4 !== 0) {
                                var curve = w3 < w4 ? c3 : c4,
                                    nextCurve = operator(curve._segment1._winding) ? curve : w3 < w4 ? c4 : c3,
  ...
  [0, 1, 2]
     ...
     b * (1 - s * (1 - f))
        ...
        return Base.each(types, function (properties, type) {
            ...
            _convert: function (type) {
                var converter;
                return this._type === type ? this._components.slice() : (converter = converters[this._type + '-' + type]) ? converter.apply(this, this._components) : converters['rgb-' + type].apply(this,
    ...
    Base.each(Color._types, function (properties, type) {
        var ctor = this[Base.capitalize(type) + 'Color'] = function (arg) {
            var argType = arg != null && typeof arg,
                components = argType === 'object' && arg.length != null ? arg : argType === 'string' ? null : arguments;
    ...
    var DomElement = new function () {
        ...
        return {
            ...
            get: function (el, key) {
                return el ? special.test(key) ? key === 'value' || typeof el[key] !== 'string' ? el[key] : true : key in translated ? el[translated[key]] : el.getAttribute(key) : null;
            },
    ...
    var CanvasView = View.extend({
        ...
        initialize: function CanvasView(project, canvas) {
            if (!(canvas instanceof HTMLCanvasElement)) {
                ...
                if (size.isZero())
                    throw new Error(
                        'Cannot create CanvasView with the provided argument: ' + canvas);
   ...
   'onKeyDown', 'onKeyUp'],
        ...
        _handleEvent: function (type, point, event) {
            ...
            case 'mousedrag':
                ...
                break;
            case 'mouseup':
                if (!point.equals(this._point) && this._updateEvent('mousedrag', point, this.minDistance,
        ...
        function importGroup(node, type, isRoot, options) {
            ...
            for (var i = 0, l = nodes.length; i < l; i++) {
                var childNode = nodes[i],
                    child;
                if (childNode.nodeType === 1 && (child = importSVG(childNode, false, options)) && !(child instanceof Symbol))
                    children.push(child);
            }
